...
#include "PiSmmCpuStandaloneSmm.h"
  ...
  )
{
  ...
  //
  DEBUG ((EFI_D_ERROR, "Patch ASM code template with current CR0, CR3, and CR4 values\n"));  
  ...
  //
  DEBUG ((EFI_D_ERROR, "Patch GDTR for SMM base relocation\n"));  
  ...
  //
  DEBUG ((EFI_D_ERROR, "Backup original contents at address 0x38000\n"));  
  ...
  //
  DEBUG ((EFI_D_ERROR, "Load image for relocation\n"));  
  ...
  DEBUG ((EFI_D_ERROR, "Relocate SM bases for all APs\n"));
  ...
  //
  DEBUG ((EFI_D_ERROR, "Relocate SM bases for BSP\n"));
  ...
  //
  DEBUG ((EFI_D_ERROR, "Restore contents at address 0x38000\n"));  
  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "PiCpuStandaloneSmmEntry - in SMM\n"));
  ...
  //
  // Use MP Hob to retrieve the number of processors and number of enabled processors
  //
  Status = GetNumberOfProcessorsFromMpHob (&mNumberOfCpus, &NumberOfEnabledProcessors);
  ASSERT_EFI_ERROR (Status);
  ASSERT (mNumberOfCpus <= PcdGet32 (PcdCpuMaxLogicalProcessorNumber));
  ...
  for (Index = 0; Index < mMaxNumberOfCpus; Index++) {
    ...
    if (Index < mNumberOfCpus) {
      Status = GetProcessorInfoFromMpHob (Index, &gSmmCpuPrivate->ProcessorInfo[Index]);
      ASSERT_EFI_ERROR (Status);
  ...
  //
  DEBUG ((EFI_D_INFO, "Allocate SMI Stacks for all processors\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, "Set SMI stack for SMM base relocation\n"));
  gSmmInitStack = (UINTN) (Stacks + mSmmStackSize - sizeof (UINTN));

  //
  // Initialize IDT
  //
  DEBUG ((EFI_D_INFO, "Initialize IDT\n"));
  InitializeSmmIdt ();

  //
  // Relocate SMM Base addresses to the ones allocated from SMRAM
  //
  DEBUG ((EFI_D_INFO, "Relocate SMM Base addresses to the ones allocated from SMRAM\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, "Call hook for BSP to perform extra actions in normal mode\n"));
  SmmCpuFeaturesSmmRelocationComplete ();

  //
  // SMM Time initialization
  //
  DEBUG ((EFI_D_INFO, "SMM Time initialization\n"));
  InitializeSmmTimer ();

  //
  // Initialize MP globals
  //
  DEBUG ((EFI_D_INFO, "Initialize MP globals\n"));
  Cr3 = InitializeMpServiceData (Stacks, mSmmStackSize);

  //
  // Fill in SMM Reserved Regions
  //
  DEBUG ((EFI_D_INFO, "Fill in SMM Reserved Regions\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, "Install the SMM Configuration Protocol\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, "Install the SMM CPU Protocol\n"));
  Status = gMmst->MmInstallProtocolInterface (
                    &mSmmCpuHandle,
  ...
  //
  DEBUG ((EFI_D_INFO, "Expose address of CPU Hot Plug Data structure\n"));
  if (FeaturePcdGet (PcdCpuHotPlugSupport)) {
  ...
  //
  DEBUG ((EFI_D_INFO, "Initialize SMM CPU Services Support\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, "register SMM Ready To Lock Protocol notification\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, "Initialize SMM Profile feature\n"));
  InitSmmProfile (Cr3);

  //
  // Patch SmmS3ResumeState->SmmS3Cr3
  //
  DEBUG ((EFI_D_INFO, "Patch SmmS3ResumeState->SmmS3Cr3\n"));
  InitSmmS3Cr3 ();
  ...
  )
{
  ...
  SmramRangeCount = DescriptorBlock->NumberOfSmmReservedRegions;
  SmramRanges     = DescriptorBlock->Descriptor;
  ...
  *SmrrSize = (UINT32)CurrentSmramRange->PhysicalSize;
  ...
  //
  // Align TSEG. (Work-around if platform does not report full SMRAM.)
  //
  if (*SmrrSize != GetPowerOfTwo32 (*SmrrSize)) {
    DEBUG ((EFI_D_WARN, "SMRR unaligned Size: 0x%x\n", *SmrrSize));
    *SmrrSize = GetPowerOfTwo32 (*SmrrSize) * 2;
  }
...
EFI_STATUS
GetNumberOfProcessorsFromMpHob (
  ...
  )
{
  ...
  if (GuidHob == NULL) {
    DEBUG ((EFI_D_INFO, "GetGetNumberOfProcessorsFromMpInfo: MpInformationHob not found!\n"));
    ASSERT(FALSE);
  ...
  *NumberOfProcessors = (UINTN)MpInfo->NumberOfProcessors;
  *NumberOfEnabledProcessors = (UINTN)MpInfo->NumberOfEnabledProcessors;
  DEBUG ((EFI_D_INFO, "GetGetNumberOfProcessorsFromMpInfo: CpuCount - 0x%x\n", *NumberOfProcessors));
  return EFI_SUCCESS;
}

EFI_STATUS
GetProcessorInfoFromMpHob (
  ...
  )
{
  ...
  GuidHob = GetFirstGuidHob (&gMpInformationHobGuid);
  if (GuidHob == NULL) {
    DEBUG ((EFI_D_INFO, "GetGetProcessorInfoFromMpInfo: MpInformationHob not found!\n"));
    ASSERT(FALSE);
    if (ProcessorNumber > 1) {
      return EFI_INVALID_PARAMETER;
    }
    ProcessorInfoBuffer->ProcessorId = GetApicId ();
    ...
    ProcessorInfoBuffer->Location.Core    = (UINT32)(ProcessorInfoBuffer->ProcessorId >> 1);
    ProcessorInfoBuffer->Location.Thread  = (UINT32)(ProcessorInfoBuffer->ProcessorId & 0x1);
    return EFI_SUCCESS;
  }

  MpInfo       = GET_GUID_HOB_DATA (GuidHob);
  if (ProcessorNumber > MpInfo->NumberOfProcessors) {
    return EFI_INVALID_PARAMETER;
  }
  CopyMem (ProcessorInfoBuffer, &MpInfo->ProcessorInfoBuffer[ProcessorNumber], sizeof(EFI_PROCESSOR_INFORMATION));
  DEBUG ((EFI_D_INFO, "GetGetProcessorInfoFromMpInfo: ApicIds[%d] - 0x%x\n", ProcessorNumber, ProcessorInfoBuffer->ProcessorId));
  return EFI_SUCCESS;
}
