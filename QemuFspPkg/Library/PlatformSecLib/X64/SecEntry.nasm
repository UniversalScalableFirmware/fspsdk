...
PlatformInitialization:
  ...
  ;
  mov     rdx, PCH_RCBA_BASE_ADDRESS + 0x3404
  mov     al,  0x80
  mov     [rdx], al
...
ASM_PFX(SecCarInit):
  ...
  mov  rax, ASM_PFX(AsmGetFspInfoHeader)
...
.1:
  ...
  ;
   ...
   lea   rax, [MtrrInitTable]
   add   rbx, rax

   xor   eax, eax                       ; Clear the low dword to write
   xor   edx, edx                       ; Clear the high dword to write
   lea   rsi, [MtrrInitTable]

InitMtrrLoop:
   add   ebx, -2
   movzx ecx, WORD [rbx] ; ecx <- address of mtrr to zero
   wrmsr
   cmp   rsi, rbx
  ...
  mov     edi, dword [rax + 0x2C]   ; Code regin base
  mov     eax, dword [rax + 0x30]   ; Code regin size
  cmp     eax,  0
...
InvalidParameter:
  mov     rax, 0x8000000000000002              ; RETURN_INVALID_PARAMETER
