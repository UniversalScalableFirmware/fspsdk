  ...
  )
{
  ...
  Descriptor = FspGetResourceDescriptorByOwner(&gFspReservedMemoryResourceHobTsegGuid);
  DEBUG ((DEBUG_ERROR, "mQ35Tseg Bytes = %x.\n", Descriptor->ResourceLength)); 
  mQ35TsegMbytes = (UINT16) (Descriptor->ResourceLength >> 20);
  DEBUG ((DEBUG_ERROR, "mQ35Tseg Mbytes = %x.\n", mQ35TsegMbytes)); 
}
